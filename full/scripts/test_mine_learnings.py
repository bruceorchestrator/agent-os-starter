...
Run: `pytest full/scripts/test_mine_learnings.py` (or from an installed project, `pytest scripts/`).
...
def test_ac7_append_guard_rejects_learnings_path(tmp_path):
    bad = tmp_path / "memory" / "learnings" / "mistakes.md"
    bad.parent.mkdir(parents=True)
    with pytest.raises(ValueError):
        ml.append_to_queue(bad, "## block\n")
    assert not bad.exists(), "guard must not have created the file"
    # Guard keys on the memory/learnings/ DIR, not the basename: a mistakes.md-named
