...
Writes CANDIDATES to memory/inbox/learnings-candidates.md ONLY — never to memory/learnings/.
...
DEFAULT_QUEUE = REPO_ROOT / "memory/inbox/learnings-candidates.md"
DEFAULT_MISTAKES = REPO_ROOT / "memory/learnings/mistakes.md"
...
Populated by `/mine-learnings` (engine: `scripts/mine_learnings.py`). Each entry below is a
**CANDIDATE** mined from a session transcript — NOT an approved learning. Nothing here is
auto-promoted. You review each via `/mine-learnings`: PROMOTE → `memory/learnings/mistakes.md`
...
def append_to_queue(queue_path, md: str) -> Path:
    ...
    if "memory/learnings" in p.as_posix():  # HARD GUARD — never write curated learnings.
