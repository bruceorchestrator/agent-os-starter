...
`/mine-learnings` reads a session transcript and proposes candidate entries into `memory/inbox/learnings-candidates.md`.
You review each one and promote it here (or discard). The miner never writes to this directory — promotion is always
human-in-the-loop. See `agent-os/skills/mine-learnings/SKILL.md`.
