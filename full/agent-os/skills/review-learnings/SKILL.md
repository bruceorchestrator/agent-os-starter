...
description: Curate the memory/learnings/ directory — group repeated mistakes, promote to patterns.md, archive stale entries to archive.md. Proposes changes, NEVER auto-applies. Use when user says "review learnings", "curate learnings", "audit mistakes", "promote patterns", or when learnings/mistakes.md exceeds ~300 lines (size trigger from /morning or /lint).
allowed-tools: Read Write Edit Bash Grep
---

# /review-learnings

Curate `memory/learnings/` — keep `mistakes.md` short, active, and signal-dense. Move old or redundant entries
...
- `/morning` or `/lint` flags `learnings/mistakes.md` > 300 lines
...
## The learnings/ directory (read this first)

```
daily/                         = raw event log, append-only (never curate)
memory/learnings/mistakes.md   = active corrections (curated, ~300 lines, the hot file)
memory/learnings/patterns.md   = repeated mistakes compressed into durable rules
memory/learnings/decisions.md  = decision log (what we chose and why)
memory/learnings/constraints.md= hard always/never rules (rarely touched)
memory/learnings/archive.md    = old lessons + wins, searchable but NOT auto-loaded
```
...
Read `memory/learnings/mistakes.md` and `memory/learnings/patterns.md` end-to-end. Note the `mistakes.md` line count.
...
**PROMOTE:**
- Add the new rule to `learnings/patterns.md`
- Move the source MISTAKE entries from `mistakes.md` to `learnings/archive.md` (append, original date preserved)

**ARCHIVE:**
- Move entry from `mistakes.md` to `learnings/archive.md` (append, original date preserved)
...
`memory/learnings/mistakes.md` is loaded into AI sessions for pattern recall. Every line costs tokens AND attention.
