---
name: ingest
description: Reflexive Karpathy LLM-wiki Ingest. Save external source (URL, gist, YouTube, repo, article, PDF, file path, or pasted text) to memory/raw/, extract topics, update memory/wiki/, and log in memory/daily/. Use when user shares a URL/video/article/repo/gist/transcript, drops a file path, pastes a large block of text, or says "ingest this", "save this", "remember this article", "add to memory" (or equivalent phrase in the user's preferred language). Do NOT use for one-off lookups where the user doesn't want to keep the content.
---
...
This is the single most important skill in the wiki — without it, the wiki never grows.
...
- Says: "ingest", "save this", "remember this", "add to memory" (or equivalent in any language)

Do NOT trigger for:

- Simple lookups ("what does this say?") where the user doesn't want to keep it
- Casual references in conversation
- Things already in memory (search wiki first; if found, cite instead)
...
- **Existing topic** → update relevant section. If new source contradicts existing claims, add a note callout with both views and dates.
